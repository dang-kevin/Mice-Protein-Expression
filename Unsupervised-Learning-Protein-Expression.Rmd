...
mouse <- read.csv("Data_Cortex_Nuclear.csv")
...
According to Appendix Figure \ref{fig:HCsil} the optimal number of clusters is 3. In Table \ref{tab:HC}, cluster 3 contains one distinct class `c-CS-s`. This class represents control mice that were stimulated to learn and injected with saline.

In Figure \ref{fig:HCheat}, we see that the pPKCG protein has relatively high expression levels in clusters 1 and 2, and a low expression level in cluster 3. Conversely, the Bcatenin, ERK, and NR2A proteins have high expression levels in cluster 3 and lower expression levels in 1 and 2. These four proteins along with several others may have played a key role in creating the distinct cluster.
...
# There appear to be 3 distinct clusters: 6,7,8.
cbind(Cluster = 1:3, hc.tab) %>%
  kbl(caption = "\\label{tab:HC}Hierarchical Clustering with Complete Linkage, Breakdown by Class") %>%
  add_header_above(c(" " = 1, "Class" = 8)) %>%
  kable_styling(latex_options = "hold_position")
```


```{r, echo=F, fig.width=8, fig.height=10, fig.cap="\\label{fig:HCheat}Heatmap of Protein Expression Levels by Cluster, Hierarchical Clustering"}
...
The optimal number of clusters for k-means is also 3 (by Appendix Figure \ref{fig:KMsil}). The results are in Table \ref{tab:KM}, and we can see that there are no clusters that are associated with one particular class like in hierarchical clustering. However, Table \ref{tab:KMbehav} shows that in cluster 1 there are a total of 61 observations for mice that were stimulated to learn (`C/S`) versus 271 observations that were not stimulated to learn (`S/C`). Consequently, cluster 3 has an imbalance in the opposite direction, with 221 `C/S` observations, and 61 `S/C` observations.
...
cbind(Cluster = 1:3, km.tab) %>%
  kbl(caption = "\\label{tab:KM}K-Means Clustering, Breakdown by Class") %>%
  add_header_above(c(" " = 1, "Class" = 8)) %>%
  kable_styling(latex_options = "hold_position")
...
cbind(Cluster = 1:3, km.behav) %>%
  kbl(caption = "\\label{tab:KMbehav}K-Means Clustering, Breakdown by Behaviour") %>%
...
Similar to k-means clustering, the optimal number of clusters for PAM is also 3 (by Appendix Figure \ref{fig:PAMsil}). Interestingly, Table \ref{tab:PAM} shows an imbalance between the two behaviours (`C/S`: stimulated to learn, `S/C`: not stimulated to learn) in clusters 1 and 3. Table \ref{tab:PAMbehav} provides the numbers: in cluster 1, there are 265 observations belonging to `C/S`, and just 45 for the `S/C` behaviour. Cluster 3 shows the opposite imbalance, with 85 in `C/S` and 329 in `S/C`. The clusters formed by PAM are quite similar to the k-means clusters. Additionally, their heatmaps for protein expression levels by clusters in Figure \ref{fig:KMPAM} are almost identical with some very minor differences. One concerning detail is that there are no visible differences between clusters 1 and 3 for both methods, so it is possible that the imbalance between the two mice behaviours in these clusters may have been due to random chance.
...
cbind(Cluster = 1:3, pam.tab) %>%
  kbl(caption = "\\label{tab:PAM}Partitioning Around Medoids, Breakdown by Class") %>%
  add_header_above(c(" " = 1, "Class" = 8)) %>%
  kable_styling(latex_options = "hold_position")
...
cbind(Cluster = 1:3, pam.behav) %>%
  kbl(caption = "\\label{tab:PAMbehav}Partitioning Around Medoids, Breakdown by Behaviour") %>%
  add_header_above(c(" " = 1, "Behaviour" = 2)) %>%
  kable_styling(latex_options = "hold_position")
```


```{r, echo=F, fig.width=10, fig.height=13, fig.cap="\\label{fig:KMPAM}Heatmap of Protein Expression Levels by Cluster, K-Means & PAM"}
# Reshape the data from wide to long
...
The cumulative proportion of variance explained is shown in Figure \ref{fig:PCAvar}. While the first two components only explain 43% of the variance, 80% of variance explained is achieved at the ninth principal component. This result is quite good for a data set containing 77 proteins. Figure \ref{fig:PCAclass} visualizes the classes with just the first two principal components which is not ideal, however it is important to note that principal components do not need to be two-dimensional.

```{r, echo=F, fig.width=9, fig.cap="\\label{fig:PCAvar}Principal Component Analysis, Cumulative Proportion of Variance Explained"}
...
```{r, echo=F, fig.width=9, fig.cap="\\label{fig:PCAclass}Principal Component Analysis, first two components"}
...
data.frame(Class, Genotype, Behaviour, Treatment, Count) %>%
  kbl(caption = "\\label{tab:Mice}Mice Classes", 
      align = "l") %>%
  kable_styling(latex_options = "hold_position")
```
...
```{r, echo=F, fig.width=8, fig.cap="\\label{fig:HCsil}Silhouette Width, Hierarchical Clustering"}
hc.sil.width <- c()
for(i in 1:9){
...
```{r, echo=F, fig.width=8, fig.cap="\\label{fig:KMsil}Silhouette Width, K-Means Clustering"}
## K-means ##
...
```{r, echo=F, fig.width=8, fig.cap="\\label{fig:PAMsil}Silhouette Width, Partitioning Around Medoids"}
## PAM ##
set.seed(8)
